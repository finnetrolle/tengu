...
- **Статус:** Blocked
- **Приоритет:** High
- **Зависит от:** пересмотра и поставки контракта metadata после отмены TNG-01-02
...
## Блокировка после отмены TNG-01-02

2026-09-15 задача, поставлявшая `effect` и `confirmationPolicy`, отменена,
её файл удалён. Эти поля отсутствуют в текущем protocol. До реализации
пересмотреть предпосылки этой issue и согласовать поставку metadata.
[TNG-05](../issue_05_agent_tool_discovery.md) исследует передачу агенту
информации об инструментах; её завершение само по себе этот блокер не снимает.
Контракт ниже сохраняется как план approvals, требующий указанной предпосылки.
