...
Каждая импортированная команда явно содержит поля `effect` и
`confirmationPolicy` из
[TNG-01-02](../issues/epic_01/issue_01_02_command_effect_metadata.md).
Фрагменты публичных command descriptors после импорта:
...
- Неуказанный эффект оставляет операцию вне manifest с причиной в отчёте.
  Совместимые defaults `READ/NONE` из TNG-01-02 не применяются к неизвестным
  импортированным операциям. Импортёр формирует policy по таблице выше;
...
|---|---|---|
| A. Первая команда чтения | Импорт операции получения объекта; `READ/NONE` видны в help и manifest, CLI возвращает компактный результат | TNG-01-02 |
...
Начальный этап для детализации: A, его поставка зависит от metadata TNG-01-02.
