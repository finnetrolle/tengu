...
`FlagDefaultsTest` проверяет обработку значений флагов по умолчанию: `./gradlew :toolkit:test`. Контракт также проверяется плагином и сервером (`JiraPluginTest`, `ServerRoutesTest`).
